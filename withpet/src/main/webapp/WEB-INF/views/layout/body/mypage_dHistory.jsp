			...
			<!-- 반복구간 -->
				...
				<c:choose>
					<c:when test="${fn:length(requestScope.listdto.list)==0 }">
						<br><div align="center"><b>기부 내역이 없습니다. </b></div>
					</c:when>
					<c:otherwise>
						<c:forEach var="list" items="${requestScope.listdto.list }">
							<div class="row" style="margin-left: 0px; border: 1px solid #ddd; width: 100%; margin: 0 auto; margin-bottom: 30px">
								<!-- 섬네일시작 -->
								...
								<div class="col-sm-9" style="margin: 30px 0 0 30px">
									<table class="table table-bordered">
										<tbody>
											...
											<tr>
												<td style="text-align: center; font-weight: bold">모금개설자(기관)</td>
												<td>${list.id }(${list.donationOrg })</td>
											...
											<tr>
												...
												<td>${list.dreamMoney }원</td>
												<td style="text-align: center; font-weight: bold">나의모금액</td>
												<td>${list.currentMoney}원</td>
											</tr>
											<tr>
				...
				<div class="col-sm-6" align="center">
						...
						<ul class="pagination">
								<c:if test="${pb.previousPageGroup }">
									<li class="page-item"><a class="page-link" href="${pageContext.request.contextPath}/mypage_dHistory.do?pageNo=${pb.startPageOfPageGroup-1}">&laquo;</a></li>
								</c:if>
								<c:forEach var="i" begin="${pb.startPageOfPageGroup }" end="${pb.endPageOfPageGroup }">
									<c:choose>
										<c:when test="${pb.nowPage!=i }">
											<li class="page-item">
												<a class="page-link" href="${pageContext.request.contextPath}/mypage_dHistory.do?pageNo=${i}">${i}</a></li>	
										</c:when>
										<c:otherwise>
								...
								<c:if test="${pb.nextPageGroup}">
									<li class="page-item"><a class="page-link" href="${pageContext.request.contextPath}/mypage_dHistory.do?pageNo=${pb.endPageOfPageGroup+1}">&raquo;</a></li>
								</c:if>
						</ul>
					</div>
				<div class="col-sm-3"></div>
