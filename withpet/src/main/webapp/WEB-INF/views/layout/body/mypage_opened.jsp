			...
			<div class="tab-pane fade in active show">
				...
				<c:choose>
					<c:when test="${fn:length(requestScope.listdto.list)==0 }">
						<br><div align="center"><b>개설한 모임 내역이 없습니다. </b></div>
					</c:when>
					<c:otherwise>
						<c:forEach var="listdto" items="${requestScope.listdto.list }">
							<div class="row"
						...
						<c:set var="pb" value="${requestScope.listdto.pagingBean }"/>
							<ul class="pagination">
								<c:if test="${pb.previousPageGroup }">
									<li class="page-item"><a class="page-link" href="${pageContext.request.contextPath}/mypage_opened.do?pageNo=${pb.startPageOfPageGroup-1}">&laquo;</a></li>
								</c:if>
								<c:forEach var="i" begin="${pb.startPageOfPageGroup }" end="${pb.endPageOfPageGroup }">
									<c:choose>
										<c:when test="${pb.nowPage!=i }">
											<li class="page-item">
												<a class="page-link" href="${pageContext.request.contextPath}/mypage_opened.do?pageNo=${i}">${i}</a></li>	
										</c:when>
										<c:otherwise>
								...
								<c:if test="${pb.nextPageGroup}">
									<li class="page-item"><a class="page-link" href="${pageContext.request.contextPath}/mypage_opened.do?pageNo=${pb.endPageOfPageGroup+1}">&raquo;</a></li>
								</c:if>
							</ul>
						</div>
					<div class="col-sm-3"></div>
