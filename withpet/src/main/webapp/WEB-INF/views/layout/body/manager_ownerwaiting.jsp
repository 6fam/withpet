	...
	<div class="row"
		...
		<div class="col-sm-12">
			<ul class="nav nav-tabs">
				...
				<li class="nav-item"><a class="nav-link active" href="manager_ownerwaiting.do?pageNo=1">견주대기자</a></li>
			</ul>
			<div id="myTabContent" class="tab-content">
				<div class="tab-pane fade in active show" id="allmember"
					...
					<div class="row" style="margin-left: 0px">
						...
						<form action="${pageContext.request.contextPath}/managerdogmompermit.do"
								method="post">
								<sec:csrfInput />
							<table class="table table-hover table-bordered">
								...
								<tbody>
									<c:forEach items="${requestScope.standbylist.list}" var="ml">
									<tr>
										<td>${ml.dogDTO.id}</td>
										<td>${ml.nick}</td>
										...
										<td><button type="submit"
														...
														value="${ml.dogDTO.id}">승인</button></td>
									</tr>
									</c:forEach>
								</tbody>
							</table>
					...
					<div class="row" style="margin-left: 0px">
						...
						<div class="col-sm-6">
							<div>
								<c:set value="${requestScope.standbylist.pagingBean}" var="pp" />

								<ul class="pagination">
									<c:if test="${pp.previousPageGroup }">
										<li class="page-item"><a class="page-link"
											href="${pageContext.request.contextPath}/manager_ownerwaiting.do?pageNo=${pp.startPageOfPageGroup-1}">&laquo;</a></li>
									</c:if>
									<c:forEach var="i" begin="${pp.startPageOfPageGroup}"
										...
										<c:choose>
											<c:when test="${pp.nowPage!=i }">
												<li class="page-item"><a class="page-link"
													href="${pageContext.request.contextPath}/manager_ownerwaiting.do?pageNo=${i}">${i}</a></li>
											</c:when>
											<c:otherwise>
									...
									<c:if test="${pp.nextPageGroup}">
										<li class="page-item"><a class="page-link"
											href="${pageContext.request.contextPath}/manager_ownerwaiting.do?pageNo=${pp.endPageOfPageGroup+1}">&raquo;</a></li>
									</c:if>
								</ul>
							</div>
						</div>
